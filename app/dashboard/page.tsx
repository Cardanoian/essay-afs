...
import { getAssignments, getEvaluations, getASubmission, getESubmission, getUserFullInfo } from "../lib/api";
...
export default function DashboardPage() {
  ...
  const handleAddClass = async (e: React.FormEvent) => {
    ...
    setLoadingClass(true);
    try {
      await addClass({ name: className, grade: "" });
      await fetchClasses();
      await fetchUserInfo();
      setClassName("");
    } finally {
  ...
  useEffect(() => {
    const fetchAssignments = async () => {
      try {
        ...
        // 피드백 가이드 완료 여부 검사
        const complete = userInfo?.feedback_guide &&
          Object.keys(userInfo.feedback_guide).length === 5 &&
          Object.values(userInfo.feedback_guide).every(item =>
            item && typeof item === 'object' && 'studentExample' in item && 'teacherFeedback' in item
          );
        ...
        const assignmentsWithCounts = await Promise.all(
          allAssignments.map(async (a) => {
            const { data: submissions } = await getASubmission({ assignment_id: a.id });
            const inProgress = submissions.filter((s: any) => s.status === "in_progress").length;
            const firstSubmitted = submissions.filter((s: any) => s.status === "first_submitted").length;
            const feedbackDone = submissions.filter((s: any) => s.status === "feedback_done").length;
            const finalSubmitted = submissions.filter((s: any) => s.status === "final_submitted").length;
            return {
              ...a,
              inProgress,
              firstSubmitted,
        ...
        const evaluationsWithCounts = await Promise.all(
          allEvaluations.map(async (e) => {
            const { data: submissions } = await getESubmission({ evaluation_id: e.id });
            const inProgress = submissions.filter((s: any) => s.status === "in_progress").length;
            const finalSubmitted = submissions.filter((s: any) => s.status === "submitted").length;
            return {
              ...e,
              inProgress,
              finalSubmitted
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-2xl shadow-md p-6 border border-purple-100">
          ...
          {user?.classes && user.classes.length > 0 ? (
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-green-600 font-bold text-xl">학급설정 완료</span>
              </div>
            </div>
          ) : (
            <div className="space-y-3">
              <p className="text-gray-600">아직 학급이 없습니다. 학급을 추가해 주세요!</p>
            </div>
          )}
          <button
          ...
          <Modal open={isClassModalOpen} onClose={() => { setIsClassModalOpen(false); fetchUserInfo(); }} className="max-w-lg">
            <h2 className="text-2xl font-bold text-purple-700 mb-4 text-center">학급 관리</h2>
            <form onSubmit={handleAddClass} className="flex gap-2 mb-4">
              ...
              />
              <button type="submit" className="bg-purple-600 text-white rounded-lg px-4 py-2 font-bold hover:bg-purple-700 transition">추가</button>
            </form>
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {loadingClass ? (
                <div className="text-center text-purple-400">로딩 중...</div>
              ) : classes.length === 0 ? (
                <div className="text-center text-gray-400">등록된 학급이 없습니다.</div>
              ) : (
                classes.map(cls => (
                  <div key={cls.id} className="flex items-center gap-2 bg-purple-50 rounded-lg px-4 py-2">
                    {editClassId === cls.id ? (
                      <form onSubmit={handleEditClassSubmit} className="flex gap-2 flex-1">
                        <input
                          type="text"
                          value={editClassName}
                          ...
                          autoFocus
                        />
                        <button type="submit" className="bg-green-500 text-white rounded-lg px-3 py-1 font-bold hover:bg-green-600 transition">저장</button>
                        <button type="button" onClick={() => setEditClassId(null)} className="bg-gray-300 text-gray-700 rounded-lg px-3 py-1 font-bold hover:bg-gray-400 transition">취소</button>
                      </form>
                    ) : (
                      <>
                        <span className="flex-1 font-semibold text-purple-800">{cls.name}</span>
                        <button onClick={() => handleEditClass(cls.id)} className="bg-yellow-400 text-white rounded-lg px-3 py-1 font-bold hover:bg-yellow-500 transition">수정</button>
                        <button onClick={() => handleDeleteClass(cls.id)} className="bg-red-500 text-white rounded-lg px-3 py-1 font-bold hover:bg-red-600 transition">삭제</button>
                      </>
                    )}
                  </div>
                ))
        ...
        <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-2xl shadow-md p-6 border border-orange-100">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-xl font-bold text-orange-700">AI 가이드 설정</h2>
          </div>
          {isFeedbackGuideComplete ? (
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-green-600 font-bold text-xl">AI 가이드 설정 완료</span>
              </div>
            </div>
          ) : (
            <div className="space-y-3">
              <p className="text-gray-600">AI가 선생님의 피드백 스타일을 이해할 수 있도록 가이드를 설정해주세요!</p>
            </div>
          )}
          <button
            onClick={() => {
      ...
      <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl shadow-md p-8 flex flex-col gap-4 border border-blue-100 mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className="ml-auto text-lg font-bold text-blue-600 bg-white rounded-full px-4 py-1 shadow">{ongoingAssignments.length}</span>
        </div>
        <ul className="divide-y divide-blue-100 bg-white rounded-xl shadow p-4">
          {loadingAssignments ? (
            ...
            <li className="py-4 text-center text-gray-400">진행중인 과제가 없습니다.</li>
          ) : (
            ongoingAssignments.map((a, i) => (
              <li key={i} className="py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <span className="font-semibold text-gray-800 text-lg">{a.name}</span>
                  <span className="ml-2 text-xs bg-blue-100 text-blue-700 rounded-full px-2 py-0.5 font-bold">{a.className}</span>
                  <div className="flex gap-4 mt-2 text-sm">
                    <span className="text-gray-700">초고 작성 중: <span className="font-bold text-black-700">{a.inProgress}</span></span>
                    <span className="text-gray-700">1차 작성 완료: <span className="font-bold text-blue-700">{a.firstSubmitted}</span></span>
                    <span className="text-gray-700">피드백 제공: <span className="font-bold text-purple-700">{a.feedbackDone}</span></span>
                    <span className="text-gray-700">최종 제출: <span className="font-bold text-green-700">{a.finalSubmitted}</span></span>
                  </div>
                </div>
                <div className="flex gap-2 min-w-[200px] justify-end">
                  <button
                    ...
                    onClick={() => router.push(`/a_dist/assignment/${encodeAssignmentToken(a.id, a.class_id)}`)}
                  >
                    배부 페이지
                  </button>
                  <button
                    ...
                    onClick={() => router.push(`/a_dist/manage_submit/${encodeAssignmentToken(a.id, a.class_id)}`)}
                  >
                    관리 페이지
                  </button>
                </div>
      ...
      <div className="bg-gradient-to-br from-pink-50 to-pink-100 rounded-2xl shadow-md p-8 flex flex-col gap-4 border border-pink-100">
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className="ml-auto text-lg font-bold text-pink-600 bg-white rounded-full px-4 py-1 shadow">{ongoingEvaluations.length}</span>
        </div>
        <ul className="divide-y divide-pink-100 bg-white rounded-xl shadow p-4">
          {loadingEvaluations ? (
            <li className="py-4 text-center text-gray-400">로딩 중...</li>
          ) : ongoingEvaluations.length === 0 ? (
            <li className="py-4 text-center text-gray-400">진행중인 평가가 없습니다.</li>
          ) : (
            ongoingEvaluations.map((e, i) => (
              <li key={i} className="py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <span className="font-semibold text-gray-800 text-lg">{e.name}</span>
                  <span className="ml-2 text-xs bg-pink-100 text-pink-700 rounded-full px-2 py-0.5 font-bold">{e.className}</span>
                  <div className="flex gap-4 mt-2 text-sm">
                    <span className="text-gray-700">평가 작성 중: <span className="font-bold text-black-700">{e.inProgress}</span></span>
                    <span className="text-gray-700">평가 완료: <span className="font-bold text-pink-700">{e.finalSubmitted}</span></span>
                  </div>
                </div>
                <div className="flex gap-2 min-w-[200px] justify-end">
                  <button
                    ...
                    onClick={() => router.push(`/e_dist/evaluation/${encodeEvaluationToken(e.id, e.class_id)}`)}
                  >
                    배부 페이지
                  </button>
                  <button
                    ...
                    onClick={() => router.push(`/e_dist/manage_submit/${encodeAssignmentToken(e.id, e.class_id)}`)}
                  >
                    관리 페이지
                  </button>
                </div>
      ...
      {/* 학급 삭제 확인 모달 */}
      <Modal open={!!deleteClassId} onClose={() => { setDeleteClassId(null); setDeleteClassName(""); }} className="max-w-md">
        <div className="p-6 text-center">
          ...
          <h3 className="text-xl font-bold text-red-600 mb-2">정말 삭제하시겠습니까?</h3>
          <p className="mb-6 text-gray-700">학급 <span className="font-bold text-purple-700">{deleteClassName}</span>을(를) 삭제하면 복구할 수 없습니다.<br/>이 작업은 되돌릴 수 없습니다.</p>
          <div className="flex gap-4 justify-center">
            <button onClick={handleDeleteClassConfirm} className="bg-red-600 hover:bg-red-700 text-white font-bold px-6 py-2 rounded-lg shadow">삭제</button>
            <button onClick={() => { setDeleteClassId(null); setDeleteClassName(""); }} className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold px-6 py-2 rounded-lg shadow">취소</button>
          </div>
        </div>
      </Modal>
    </div>
