...
const dummyAssignments: Assignment[] = [
  { id: 1, name: "글쓰기 과제1", type: "교육활동", className: "1학년 1반" },
  { id: 2, name: "글쓰기 평가", type: "평가활동", className: "2학년 2반" },
];
...
export default function AssignmentsPage() {
  ...
  const handleAddAssignment = (e: React.FormEvent) => {
    ...
    setAssignments([
      ...assignments,
      { id: Date.now(), name, type, className, status: "pending", started_at: null, completed_at: null },
    ]);
    setName("");
    setType("교육활동");
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-extrabold mb-6 text-indigo-700 tracking-tight">과제 관리</h1>
      <button
      ...
      <div className="overflow-x-auto rounded-xl shadow">
        <table className="w-full bg-white rounded-xl">
          ...
          <tbody>
            {assignments.map(a => (
              <tr key={a.id} className={`border-t hover:bg-indigo-50 ${getRowColor(a.status)}`}>
                <td className="p-3">{a.name}</td>
                <td className="p-3">{a.type}</td>
                <td className="p-3">{a.className}</td>
                <td className="p-3">
                  {a.status === "pending" && <span className="text-gray-600 font-semibold">대기중</span>}
                  {a.status === "in_progress" && <span className="text-blue-700 font-semibold">진행중</span>}
                  {a.status === "completed" && <span className="text-green-700 font-semibold">완료</span>}
                </td>
                <td className="p-3">
                  {a.status === "pending" ? (
                    <button
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)}>
        ...
        <form onSubmit={handleAddAssignment} className="flex flex-col gap-4">
          ...
          <button type="submit" className="bg-indigo-600 text-white rounded-lg py-3 font-bold text-lg shadow hover:bg-indigo-700 transition">추가</button>
        </form>
      </Modal>
      {/* 배부 확인 모달 */}
      <Modal open={showConfirm} onClose={() => setShowConfirm(false)}>
        <h2 className="text-xl font-bold mb-4 text-blue-700">학생들에게 배부하시겠습니까?</h2>
