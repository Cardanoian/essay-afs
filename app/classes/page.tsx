...
export default function ClassesPage() {
  ...
  useEffect(() => {
    if (selectedClass) {
      ...
      getStudents(selectedClass)
        .then((res) => {
          const sorted = res.data.sort((a: Student, b: Student) => a.number - b.number);  // ✅ number 기준 오름차순 정렬
          setStudents(sorted);
        })
        .finally(() => setLoading(false));
    } else {
  ...
  const getGradeOptions = () => {
    if (user?.school_level === "초등학교") {
      return [1, 2, 3, 4, 5, 6];
    }
    if (user?.school_level === "중학교" || user?.school_level === "고등학교") {
      return [1, 2, 3];
    }
    return [];
  };
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    const existingStudent = students.find(student => student.number === Number(studentNumber));
    if (existingStudent) {
    ...
    try {
      ...
      const sorted = res.data.sort((a: Student, b: Student) => a.number - b.number);
      setStudents(sorted);
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-extrabold mb-6 text-blue-700 tracking-tight">학생 관리</h1>
      <div className="flex gap-4 mb-6 flex-wrap">
        {classes.map(cls => (
          <button
            key={cls.id}
            onClick={() => setSelectedClass(cls.id)}
            className={`px-4 py-2 rounded-lg border font-semibold shadow-sm transition-all ${selectedClass === cls.id ? "bg-blue-600 text-white scale-105" : "bg-white hover:bg-blue-50"}`}
          >
            {cls.name}
          </button>
        ))}
      </div>
      {loading && <div className="text-center text-blue-500 font-semibold">로딩 중...</div>}
      <div className="mt-6">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-lg font-bold mb-2 text-gray-700">
            {selectedClass ? "학생 목록" : "학급을 선택하면 학생 목록이 표시됩니다"}
          </h2>
          <div className="flex gap-2">
            <button
              onClick={() => setShowAddStudent(true)}
              disabled={!selectedClass}
              className={`rounded-lg px-5 py-2 font-bold shadow transition ${selectedClass
        ...
        <div className="overflow-x-auto rounded-xl shadow">
          <table className="w-full bg-white rounded-xl" style={{ tableLayout: 'fixed' }}>
              ...
              {students.length > 0 ? (
                students.map(stu => (
                  <tr
                    ...
                    onClick={() => router.push(`/students/${encodeStudentToken(stu.id)}`)}
                  >
                ...
                <tr>
                  <td colSpan={4} className="p-8 text-center text-gray-500">
                    {selectedClass ? "등록된 학생이 없습니다" : "학급을 선택해주세요"}
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <Modal open={showAddClass} onClose={() => setShowAddClass(false)}>
        ...
        <form onSubmit={handleAddClass} className="flex flex-col gap-4">
          ...
          <select
            value={classGrade}
            onChange={(e) => setClassGrade(`${e.target.value}학년`)}
            className="border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-400 text-lg"
          >
          ...
          <button type="submit" className="bg-blue-600 text-white rounded-lg py-3 font-bold text-lg shadow hover:bg-blue-700 transition">추가</button>
        </form>
      </Modal>
      {/* 학생 직접 추가 모달 */}
      <Modal open={showAddStudent} onClose={() => setShowAddStudent(false)}>
        <h2 className="text-xl font-bold mb-4 text-green-700">학생 직접 추가</h2>
        <form onSubmit={handleAddStudent} className="flex flex-col gap-4">
          ...
          <input
            ...
            className="border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-400 text-lg"
          />
          <button type="submit" className="bg-green-600 text-white rounded-lg py-3 font-bold text-lg shadow hover:bg-green-700 transition">추가</button>
        </form>
      </Modal>
      {/* 학생 CSV 업로드 모달 */}
      <Modal open={showUpload} onClose={() => setShowUpload(false)}>
        <h2 className="text-xl font-bold mb-4 text-purple-700">학생 CSV 업로드</h2>
        <form onSubmit={handleUpload} className="flex flex-col gap-4">
          ...
          />
          <p className="text-gray-500 text-sm">CSV 파일 형식: 번호,이름,이메일 (첫 행은 헤더)</p>
          <button type="submit" className="bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg py-3 font-bold text-lg shadow hover:from-purple-600 hover:to-pink-600 transition">업로드</button>
        </form>
      </Modal>
      ...
      <Modal open={showDeleteConfirm} onClose={() => { setShowDeleteConfirm(false); setDeleteTarget(null); }}>
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">
            <span className="font-semibold text-red-600">"{deleteTarget?.name}"</span> 학생을 삭제하시겠습니까?
          </p>
