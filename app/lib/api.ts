...
export const updateClass = (classId: number, class_info: { name?: string; grade?: string }) =>
...
// 학생 관련 함수
export const getStudents = (classId: number) => api.get(`/students/class/${classId}`);
...
export const uploadStudentsCSV = (
  classId: number, 
  file: File
) => {
...
export const createStudentAnalysis = (
  analysis_source : { 
    level: string; 
    grade: string; 
    submissions : object;
...
// 과제 관련 함수
export const getAssignments = (classId: number) => api.get(`/assignments/class/${classId}`);
export const getAssignment = (assignmentId: number) => api.get(`/assignments/${assignmentId}`);
...
export const updateAssignment = (assignmentId: number, assignment: {
  name?: string;
  guide?: string;
  condition?: string;
  class_id?: number;
...
// 평가 관련 함수
export const getEvaluations = (classId: number) => api.get(`/evaluation/class/${classId}`);
export const getEvaluation = (evaluationId: number) => api.get(`/evaluation/${evaluationId}`);
...
export const updateEvaluation = (evaluationId: number, evaluation: {
  name?: string;
  item?: string;
  criteria?: object;
  class_id?: number;
...
export const patchSubmissionFeedback = (
  ...
  api.patch("/submission/patch_feedback", { assignment_id, student_id, feedback });
