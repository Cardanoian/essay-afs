...
async def get_evaluation(
    ...
    result = await db.execute(select(models.Evaluation).where(models.Evaluation.id == evaluation_id))
...
async def update_evaluation(
    ...
    evaluation = await crud.update_evaluation(db, evaluation_id, evaluation_update, user.id)
    if not evaluation:
        raise HTTPException(status_code=404, detail="Evaluation not found")
    return evaluation
...
async def update_evaluation_status(
    ...
    evaluation = await crud.update_evaluation_status(db, evaluation_id, status_enum, started_at, completed_at)
    if not evaluation:
        raise HTTPException(status_code=404, detail="Evaluation not found")
    return evaluation
...
async def start_evaluation_for_class(
    request: schemas.EvaluationStartRequest,
    db: AsyncSession = Depends(database.get_db)
):
