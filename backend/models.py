...
class User(Base):
    ...
    classes = relationship(
        "SchoolClass", back_populates="user", cascade="all, delete-orphan"
    )
...
class Student(Base):
    ...
    eval_submission = relationship(
        "ESubmission", back_populates="student", cascade="all, delete-orphan"
    )
    assign_submission = relationship(
        "ASubmission", back_populates="student", cascade="all, delete-orphan"
    )
    assign_feedback = relationship(
        "AFeedback", back_populates="student", cascade="all, delete-orphan"
    )
    eval_feedback = relationship(
        "EFeedback", back_populates="student", cascade="all, delete-orphan"
    )
    analysis_result = relationship(
        "AnalysisResult", back_populates="student", cascade="all, delete-orphan"
    )
...
class Assignment(Base):
    ...
    assign_submission = relationship(
        "ASubmission", back_populates="assignment", cascade="all, delete-orphan"
    )
    assign_feedback = relationship(
        "AFeedback", back_populates="assignment", cascade="all, delete-orphan"
    )
...
class Evaluation(Base):
    ...
    submission = relationship(
        "ESubmission", back_populates="evaluation", cascade="all, delete-orphan"
    )
    eval_feedback = relationship(
        "EFeedback", back_populates="evaluation", cascade="all, delete-orphan"
    )
...
class ASubmission(Base):
    ...
    assignment_id = Column(
        Integer, ForeignKey("assignment.id", ondelete="CASCADE"), nullable=True
    )
    student_id = Column(
    ...
    status = Column(
        Enum(ASubmissionStatus), nullable=False, default=ASubmissionStatus.in_progress
    )
    ...
    assign_feedback = relationship(
        "AFeedback", back_populates="assign_submission", cascade="all, delete-orphan"
    )
...
class ESubmission(Base):
    ...
    submitted_at = Column(DateTime, nullable=False)
    status = Column(
        Enum(ESubmissionStatus), nullable=False, default=ESubmissionStatus.in_progress
    )

    evaluation = relationship("Evaluation", back_populates="submission")
    student = relationship("Student", back_populates="eval_submission")
    eval_feedback = relationship(
        "EFeedback", back_populates="eval_submission", cascade="all, delete-orphan"
    )
...
class AFeedback(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    assign_submission_id = Column(
        Integer, ForeignKey("assign_submission.id", ondelete="CASCADE"), nullable=False
    )
    assignment_id = Column(
        Integer, ForeignKey("assignment.id", ondelete="CASCADE"), nullable=True
    )
...
class EFeedback(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    eval_submission_id = Column(
        Integer, ForeignKey("esubmission.id", ondelete="CASCADE"), nullable=False
    )
    evaluation_id = Column(
